...
class _YouTubePreviewState extends State<YouTubePreview>
  ...
  void didUpdateWidget(YouTubePreview oldWidget) {
    ...
    if (oldWidget.video.canonicalUrl != widget.video.canonicalUrl ||
        oldWidget.cdnUrl != widget.cdnUrl) {
      _playing = false;
      _error = null;
    }
  }
  ...
  Widget build(BuildContext context) => LayoutBuilder(
    builder: (context, constraints) {
      ...
      final canPlay =
          (supportsYouTubePlayer || widget.playerBuilder != null) &&
        ...
        children: [
          ...
          AspectRatio(
            ...
            child: _playing
                ? widget.playerBuilder?.call(widget.video, _stop, _failed) ??
                      YouTubePlayer(
                        key: ValueKey(widget.video.canonicalUrl),
                        video: widget.video,
                        onStopped: _stop,
                        onError: _failed,
                      )
                : ColoredBox(
                    ...
                    child: widget.poster == null
                        ? const Center(
                            child: Icon(Icons.smart_display, size: 48),
                          )
                        : MessageMediaGate(
                            source: widget.poster,
                            trustedBaseUrl: widget.cdnUrl,
                            blockedPlaceholder: const Center(
                              child: Icon(Icons.smart_display, size: 48),
                            ),
          ...
          if (_error != null)
            Padding(
              ...
              child: Text(
                _error!,
                key: const ValueKey('youtube-playback-error'),
              ),
            ),
          if (canPlay && !_playing)
            const Padding(
