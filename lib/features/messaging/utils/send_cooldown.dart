...
enum SendCooldownKind {
  ...
  /// A server rejection whose limiter is not identified.
  rateLimited,
}
...
class SendCooldown {
  const SendCooldown({
    required this.until,
    required this.kind,
    this.uploadsOnly = false,
  });

  final bool uploadsOnly;
...
///
/// The server does not identify the limiter. Keep generic wording and scope
/// the wait to uploads only when the rejected attempt carried files outside slowmode.
SendCooldown? cooldownFromFailure({
  ...
  return SendCooldown(
    ...
    uploadsOnly: hasAttachments && slowmodeSeconds == 0,
  );
}
...
bool sendBlockedByCooldown({
  ...
  return !cooldown.uploadsOnly || hasAttachments;
}
