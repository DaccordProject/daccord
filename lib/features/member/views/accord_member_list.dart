...
class _Roster extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final sections = _buildSections(members.values.toList(), roles);
      ...
      children: [
        for (final section in sections) ...[
          ...
          for (final member in section.members)
            _MemberRow(member: member, roles: roles, cdnUrl: cdnUrl),
        ],
      ],
    );
  }
...
/// Buckets [members] under their highest hoisted role (falling back to a
/// trailing "Members" group), each bucket ordered by role position descending
/// and members sorted by display name.
...
class _MemberRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final avatarUrl = accordAvatarUrl(member.user, cdnUrl);
