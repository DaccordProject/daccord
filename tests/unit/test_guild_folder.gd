...
func test_setup_stores_folder_name() -> void:
	component.setup("MyFolder", _spaces_data())
...
func test_setup_mini_grid_creates_swatches() -> void:
	component.setup("MyFolder", _spaces_data())
	# Should have 2 swatches (one per space, up to 4)
	await get_tree().process_frame
	var swatch_count := 0
...
func test_setup_creates_space_icons() -> void:
	component.setup("MyFolder", _spaces_data())
	assert_eq(component.space_icons.size(), 2)
...
func test_toggle_expanded() -> void:
	...
	assert_true(component.space_list.visible)
...
func test_has_set_active_space_method() -> void:
	assert_true(component.has_method("set_active_space"))
...
func test_set_active_false_restores_unread() -> void:
	component.setup("MyFolder", _spaces_data_with_unread())
	component.set_active(true)
	component.set_active(false)
	assert_false(component.is_active)
	# Should restore to UNREAD since there are unread spaces
	assert_eq(component.pill.pill_state, component.pill.PillState.UNREAD)


func test_set_active_space() -> void:
	component.setup("MyFolder", _spaces_data())
	component.set_active_space("g_1")
	assert_true(component.is_active)
	assert_eq(component.pill.pill_state, component.pill.PillState.ACTIVE)
	# Check that the correct child icon is active
	assert_true(component.space_icons[0].is_active)
	assert_false(component.space_icons[1].is_active)


func test_set_active_space_switches() -> void:
	component.setup("MyFolder", _spaces_data())
	component.set_active_space("g_1")
	component.set_active_space("g_2")
	assert_false(component.space_icons[0].is_active)
	assert_true(component.space_icons[1].is_active)
