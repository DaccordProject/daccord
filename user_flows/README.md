...
| 5 | [Responsive Layout](responsive_layout.md) | Three layout modes (COMPACT/MEDIUM/FULL), sidebar drawer, hamburger button, sidebar/member/search toggles, edge-swipe gestures, panel resize handles, DPI/density-aware scaling gaps for mobile |
...
| 70 | [Server Plugins](server_plugins.md) | Per-server plugin system with scripted (sandboxed Lua via lua-gdextension) and native (full GDScript scenes with trust dialog) runtimes; ZIP bundle download with caching; lobby/role system (player/spectator); LiveKit data channels for real-time state sync, input forwarding, and file sharing; plugin trust model; chess (scripted) and NES emulator (native) worked examples |
...
| 72 | [PostgreSQL Support](postgres_support.md) | Adding Postgres as an alternative database backend: sqlx Any driver, schema translation, query dialect abstraction, migration strategy, data migration tooling |
| 73 | [Plugin Test Coverage](plugin_test_coverage.md) | Plugin system test + security audit: 173 tests across 7 test files cover model/REST/cache/gateway/routing/bundle/trust/state/canvas/context/runtime; ~63% line coverage; untested: join_activity, check_active_session, 5 REST endpoints, Lua sandbox start, UI dialogs; security: ZIP path traversal, stub signatures, trust-all bypass, BBCode injection, Lua CPU exhaustion |
| 74 | [One-Click Deploy](one_click_deploy.md) | One-click accordserver deployment on Railway, Render, Fly.io, DigitalOcean, Docker Compose; platform configs, deploy buttons, env var reference, voice/storage gaps |
| 75 | [UI Audit](ui_audit.md) | Comprehensive catalog of all 121 UI surfaces (views, dialogs, panels, overlays) for designer/UX screenshot review; responsive breakpoints, context menus, platform variants, execution plan |
| 76 | [Client MCP Server](client_mcp.md) | MCP protocol adapter wrapping the Client Test API for AI agents; bearer token auth, JSON-RPC 2.0, tool group permissions, MCP content types; gated behind Developer Mode |
...
| 81 | [Mobile Gesture Navigation](mobile_gesture_navigation.md) | Edge-swipe drawer open/close, velocity-based snap, long-press detection, layout mode transitions, Android back button / navigation history |
| 82 | [URL Sharing](url_sharing.md) | "Copy Link" context menu items on spaces, channels, and messages to generate shareable `daccord://` deep links; URL builder helpers, clipboard copy, toast feedback, receiving-side channel/message navigation |
| 83 | [Plugin Platform Services](plugin_platform_services.md) | Server-provided backend services for plugins: key-value storage, leaderboards, achievements, server-authoritative state sync, matchmaking, scheduled timers, virtual currencies, plugin announcements, cross-session statistics |
| 84 | [Activities List](activities_list.md) | Dedicated activities section in channel list: browse active sessions, join lobbies, join host voice, real-time status updates, space-level session cache |
| 85 | [Screen Sharing Audio](screen_sharing_audio.md) | Application-level audio streaming alongside screen share: frametap audio API, platform backends (PipeWire/PulseAudio/WASAPI/ScreenCaptureKit), godot-livekit bindings, daccord integration |
