...
User onboarding covers the complete first-run experience: launching daccord with no configured servers, adding the first server (including authentication), and reaching a usable state with spaces, channels, and messages displayed. It also covers subsequent launches where the previous session is restored automatically.
...
3. Client stays in `CONNECTING` mode
4. **Welcome screen appears** as a full-window overlay covering the entire UI (sidebar, channel list, user bar): animated shader background (navy-to-purple gradient with bokeh particles and sparkle shimmer), floating CPUParticles2D rising upward, staggered entrance animations for branding and feature cards
5. Welcome screen displays: "daccord" logo, tagline "Connect. Communicate. Collaborate.", three feature cards (Multi-Server, Real-Time Chat, Voice & Video), a pulsing "Add a Server" CTA button, and a "Settings" button in the bottom-left corner
...
13. Client authenticates (`GET /users/@me`), lists spaces (`GET /users/@me/spaces`), matches the space name, opens WebSocket gateway
14. `AppState.spaces_updated` fires -- sidebar auto-selects the new space
...
4. On first successful connection, `AppState.spaces_updated` fires
5. `sidebar._on_spaces_updated()` reads `Config.get_last_selection()` to restore the previous space and channel
6. If saved space still exists, it's selected; otherwise falls back to `Client.spaces[0]`
...
main_window._ready()
    ...
    -> _show_welcome_screen()
        -> WelcomeScreenScene instantiated, added to MainWindow (full-window overlay)
        -> layout_hbox hidden (sidebar + content area)
        -> connects AppState.spaces_updated -> _on_first_server_added (ONE_SHOT)
        -> welcome_screen._ready() -> _animate_entrance()
            -> staggered fade-in: logo (0.0s), tagline (0.15s), features (0.3s), CTA + settings (0.5s)
            -> CTA pulse glow loop starts after entrance

UI shows: full-window welcome screen with animated shader bg + particles (sidebar hidden)

User clicks CTA "Add a Server" (or "+" in space bar)
...
User enters URL, clicks "Add"
    ...
    -> _connect_with_token()                          (line 161)
        -> Config.add_server(url, token, space_name)  (line 169)
        -> Client.connect_server(last_index)          (line 171)
            -> GET /users/@me                         (line 143 of client)
            -> GET /users/@me/spaces                  (line 223 of client)
            -> match space_name -> space_id           (lines 239-254)
            -> client.login() (WebSocket)             (line 275)
            -> mode = Mode.LIVE                       (line 279)
            -> AppState.spaces_updated.emit()         (line 280)

        -> main_window._on_first_server_added()        (ONE_SHOT)
            -> welcome_screen.dismiss()
                -> fade out (0.3s)
                -> dismissed.emit() -> layout_hbox.visible = true
                -> queue_free()

        -> sidebar._on_spaces_updated()               (line 22 of sidebar)
            -> _startup_selection_done = true          (line 27)
            -> Config.get_last_selection() (empty)     (line 29)
            -> fallback to Client.spaces[0]            (line 42)
            -> guild_bar._on_space_pressed(space_id)   (line 46)
                -> guild_bar.space_selected.emit()     (line 79 of guild_bar)
        -> sidebar._on_space_selected()                (line 48)
            -> channel_list.load_space(space_id)       (line 51)
        ...
        -> server_added.emit(space_id)                 (line 179)
        -> guild_bar._on_server_added(space_id)        (line 92 of guild_bar)
...
Client._ready()
    ...
    for i in servers:
        connect_server(i)                              (line 92)
            -> authenticates, matches space, connects gateway
            -> AppState.spaces_updated.emit()          (line 280)

sidebar._on_spaces_updated()                           (line 22)
    Config.get_last_selection()                         (line 29)
    -> {space_id, channel_id} restored from config
    -> channel_list.pending_channel_id = saved_channel  (line 45)
    -> guild_bar._on_space_pressed(saved_space_id)      (line 46)
        -> channel_list.load_space() uses pending_channel_id
...
| `scenes/sidebar/guild_bar/guild_bar.gd` | Instantiates dialog (line 87), auto-selects new space (line 92) |
...
Config stores servers as indexed sections (`[server_0]`, `[server_1]`, ...) with a count in `[servers]`. Each entry holds `base_url`, `token`, `space_name`, `username`, and `password`. The config is encrypted with a key derived from `"daccord-config-v1"` + the user data directory path (line 18-19).
...
3. `GET /users/@me/spaces` -- lists the user's spaces (line 223), matches `space_name` to find `space_id` (lines 239-254)
4. Fetches full space details (line 256)
5. Opens the WebSocket gateway via `client.login()` (line 275)
6. Sets `mode = Mode.LIVE` (line 279), emits `AppState.spaces_updated` (line 280)
...
`sidebar._on_spaces_updated()` (line 22) is the critical startup hook. It guards with `_startup_selection_done` (line 23) so it only fires once. If `Client.spaces` is empty, it returns early (line 25-26).

On first invocation with spaces available, it reads `Config.get_last_selection()` (line 29) which returns `{space_id, channel_id}`. If the saved space exists in `Client.spaces`, it's used (lines 34-38); otherwise it falls back to `Client.spaces[0]` (line 42). The saved channel ID is passed to `channel_list.pending_channel_id` (line 45), and the space is selected via `space_bar._on_space_pressed()` (line 46).

### Space bar: auto-select after adding server

After `Client.connect_server()` succeeds, the Add Server dialog emits `server_added(space_id)` (line 179 of add_server_dialog). space_bar `_on_server_added()` (line 92) receives this and calls `_on_space_pressed(space_id)` (line 94) to auto-select the newly added server, so the user immediately sees its channels.

### Welcome screen (first-run)

`main_window._ready()` checks `Config.has_servers()`. If false, it calls `_show_welcome_screen()` which instantiates the `WelcomeScreenScene`, adds it to the MainWindow root as a full-window overlay, hides `layout_hbox` (sidebar + content area), and connects `AppState.spaces_updated` to `_on_first_server_added()` as a one-shot.
...
3. **Content**: Logo, tagline, three feature cards (Multi-Server, Real-Time Chat, Voice & Video) with semi-transparent panel backgrounds, a large blurple CTA button, and a flat "Settings" button anchored to the bottom-left corner (opens user settings).

**Entrance animation** (tween-based, ~1.2s total): All content starts at `modulate.a = 0` offset 30px down. Elements stagger in with EASE_OUT CUBIC: logo (0.0s), tagline (0.15s), features (0.3s), CTA (0.5s with scale bounce from 0.9 to 1.0 via TRANS_BACK), settings button (0.5s fade-in). After entrance, the CTA gets a looping pulse glow (modulate oscillates between 1.0 and 1.1 brightness, 2s period).

**Dismiss animation**: On `_on_first_server_added()`, calls `dismiss()` which fades the entire screen out over 0.3s, emits `dismissed`, and `queue_free()`s. The callback re-shows `layout_hbox` (sidebar + content area).
...
**Channel list empty state** (channel_list.gd, lines 45-61): When a space has zero non-category channels, an `EmptyState` VBox appears. If the user has `MANAGE_CHANNELS` permission, it shows "No channels yet" / "Create your first channel to get started." with a "Create Channel" button. Otherwise: "No channels yet" / "This space doesn't have any channels yet. Check back soon!"
...
- [x] Session restore (last selected space + channel)
- [x] Auto-select newly added space
