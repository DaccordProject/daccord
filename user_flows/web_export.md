...
| `export_presets.cfg` (preset.5 `Web`) | Web export preset. `export_path="dist/build/web/Daccord.html"`, `custom_html_shell="res://dist/web/index.html"`, `variant/thread_support=false`, `variant/extensions_support=true` (required to load lua-gdextension on web — produces `Daccord.side.wasm` dlink module). Excludes `addons/godot-livekit/*`, `addons/godot_sandbox/*`. Sentry injection disabled. |
