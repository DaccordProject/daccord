...
| Voice channel restrictions | Low | Voice channels gated by `connect` / `speak` permissions should appear disabled or hidden in imposter mode. Currently voice permission checks happen in AccordStream, not through `Client.has_permission()`. |
