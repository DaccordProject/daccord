...
class _Auth extends AccordAuth {
  ...
  @override
  String? keyForBaseUrl(String baseUrl) => connected &&
      AccordServer.sameEndpoint(baseUrl, session.server.baseUrl)
      ? session.key
      : null;
  ...
  void _connect() {
    connected = true;
    ref.read(connectionsControllerProvider.notifier).register(
      session,
...
void main() {
  ...
  ProviderContainer containerFor(_Auth auth) {
    final container = ProviderContainer(overrides: [
      ...
      accordDiscoveryBrowseProvider.overrideWithValue(({
        ...
        {'space_id': 'joined-space', 'server_url': 'https://CHAT.example:443/', 'name': 'Target'},
      ])),
    ]);
  ...
  Widget host(ProviderContainer container, Widget body) => UncontrolledProviderScope(
    container: container,
    child: MaterialApp(
  ...
  for (final url in [
    ...
    testWidgets('saved account accepts $url without credentials', (tester) async {
      final auth = _Auth(session, client)..lookupGate = Completer<void>();
      final container = containerFor(auth);
      await tester.pumpWidget(host(container, Builder(
      ...
      expect(
        resolveDeepLinkDestination(pending, container.read(connectionsControllerProvider)),
  ...
  testWidgets('login discovery reuses a saved account before invoking auth callback', (tester) async {
    final auth = _Auth(session, client)..lookupGate = Completer<void>();
    final container = containerFor(auth);
    var requestedCredentials = false;
    await tester.pumpWidget(host(container, AccordDiscoveryBody(
  ...
  for (final url in [
    ...
    testWidgets('saved account joins the public space from $url', (tester) async {
  ...
  testWidgets('existing membership conflict fetches and opens its space', (tester) async {
    joinStatus = 409;
    final auth = _Auth(session, client);
    final container = containerFor(auth);
  ...
  testWidgets('discovery requests credentials only after no saved match', (tester) async {
  ...
  testWidgets('failed saved-account join stays in discovery and does not switch', (tester) async {
    joinStatus = 403;
    final auth = _Auth(session, client);
    final container = containerFor(auth);
    await tester.pumpWidget(host(container, const AccordDiscoveryBody()));
  ...
  testWidgets('signed-out invite survives login and is accepted on its account', (tester) async {
    final directory = Directory.systemTemp.createTempSync('pending-invite-login');
    ...
    final auth = _Auth(session, client, saved: false);
    final container = containerFor(auth);
    final pending = ServerUri.parseDeepLink('daccord://invite/abc123@chat.example')!;
    container.read(pendingServerJoinProvider.notifier).hold(pending);
    await tester.pumpWidget(host(container, const AccordLoginScreen(startOnCredentials: true)));
    await tester.pumpAndSettle();
