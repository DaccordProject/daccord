...
void main() {
    ...
    client = AccordClient(
      ...
      httpClient: MockClient((request) async {
        ...
        if (path == '/spaces' && request.method == 'POST') {
          ...
          data = [
            {'id': 'role', 'position': 1, 'permissions': permissions},
          ];
          ...
          if (request.method == 'POST') {
            ...
            data = channels.values
                .where((channel) => channel['space_id'] == 's')
                .toList();
          }
  ...
  test('space management is a dedicated opt-in group', () {
    ...
    expect(
      AccordSettings.defaultMcpAllowedGroups,
      isNot(contains('space_management')),
    );
    expect(
      AccordSettings.fromJson({}).mcpAllowedGroups,
      isNot(contains('space_management')),
    );
    for (final name in [
      'create_space',
  ...
  test(
    'space creation and partial update return entities and populate both caches',
    () async {
      final created = await call('create_space', {
        'name': 'Gaming',
      ...
      final updated = await call('update_space', {
        ...
        'explicit_content_filter': 'everyone',
        'nsfw_level': 'moderate',
  ...
  for (final type in ['category', 'text', 'announcement', 'forum', 'voice']) {
    test(
      'creates $type with typed fields and caches returned entity',
      () async {
        ...
        expect(
          container
              .read(accordChannelsControllerProvider(session.key, 's'))!
              .any((channel) => channel.id == 'new'),
          isTrue,
        );
      },
    );
  ...
  test(
    'updates type and clears topic/parent without losing explicit nulls',
    () async {
      ...
      expect(
        body.keys,
        containsAll(['topic', 'parent_id', 'rate_limit', 'nsfw']),
      );
  ...
  test(
    'reorders channels and refreshes cache including parent changes',
    () async {
      final result = await call('reorder_channels', {
        'space_id': 's',
        'channels': [
          {'id': 'text', 'position': 2, 'parent_id': 'category'},
        ],
      });
      expect(result['ok'], isTrue);
      final updated = (result['channels'] as List).cast<Map>().singleWhere(
        (channel) => channel['id'] == 'text',
      );
  ...
  test(
    'deletion requires explicit confirmation and removes cached channel',
    () async {
      final missing = await call('delete_channel', {
        'space_id': 's',
        'channel_id': 'text',
      });
      expect(missing['code'], 'validation_error');
      expect(writes, isEmpty);
      final result = await call('delete_channel', {
        'space_id': 's',
        'channel_id': 'text',
        'confirm': true,
      });
  ...
  test(
    'checks fresh role permissions and prevents writes for unauthorized members',
    () async {
      space['owner_id'] = 'another-user';
      final denied = await call('update_space', {
        'space_id': 's',
        'name': 'Denied',
      });
      ...
      final deniedChannel = await call('delete_channel', {
        'space_id': 's',
        'channel_id': 'text',
        'confirm': true,
      });
  ...
  test(
    'rejects cross-space targets and destructive channel type changes',
    () async {
      for (final args in [
        ...
        expect(
          (await call('update_channel', args))['code'],
          'validation_error',
        );
      }
      expect(
  ...
  test(
    'schema validation rejects coerced values, unknown fields and duplicate IDs',
    () async {
      for (final args in [
        {'space_id': 's', 'name': ''},
    ...
    intercept = (request) async => request.method == 'POST'
        ? http.Response(
            jsonEncode({
              'error': {
                'code': 'forbidden',
                'message': 'Space creation disabled',
              },
            }),
            403,
          )
  ...
  test(
    'refresh failures warn without reporting an accepted write as failed',
    () async {
      intercept = (request) async =>
          request.method == 'GET' && request.url.path.endsWith('/channels')
          ? http.Response(
              jsonEncode({
                'error': {
                  'code': 'unavailable',
  ...
  test(
    'account switch during permission fetch prevents the mutation',
    () async {
