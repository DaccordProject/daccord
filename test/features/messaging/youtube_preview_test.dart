...
Widget _host(Widget child, {double width = 440, bool ticking = true}) =>
    ProviderScope(
      child: MaterialApp(
...
class _FakePlayerState extends State<_FakePlayer> {
  ...
  Widget build(BuildContext context) =>
      const Text('official player placeholder');
}

void main() {
  testWidgets('poster and iframe each require explicit third-party consent', (
    tester,
  ) async {
    var created = 0;
    ...
    expect(
      find.textContaining('shares your connection with Google'),
      findsOneWidget,
    );
  ...
  testWidgets(
    'official playback errors restore the poster and external fallback',
    (tester) async {
  ...
  testWidgets(
    'app backgrounding destroys playback without resuming automatically',
    (tester) async {
      var disposed = 0;
      await tester.pumpWidget(
        _host(
          YouTubePreview(
            video: _video,
            playerBuilder: (_, _, _) =>
                _FakePlayer(onDisposed: () => disposed++),
          ),
        ),
      );
      await tester.tap(find.byKey(const ValueKey('youtube-play')));
  ...
  testWidgets(
    'offscreen stop callback tears down player and keeps consent prompt',
    (tester) async {
      VoidCallback? stop;
      var disposed = 0;
      await tester.pumpWidget(
  ...
  testWidgets('video changes, inactive routes and removal dispose playback', (
    tester,
  ) async {
    var disposed = 0;
    Widget preview(YouTubeVideo video) => YouTubePreview(
      video: video,
    ...
    await tester.pump();
    await tester.pumpWidget(
      _host(preview(YouTubeVideo.parse('https://youtu.be/aaaaaaaaaaa')!)),
    );
    expect(disposed, 1);
    await tester.tap(find.byKey(const ValueKey('youtube-play')));
    await tester.pump();
    await tester.pumpWidget(
      _host(
        preview(YouTubeVideo.parse('https://youtu.be/aaaaaaaaaaa')!),
        ticking: false,
      ),
    );
    expect(disposed, 2);
  ...
  testWidgets(
    'narrow columns fall back externally and cannot silently resume',
    (tester) async {
      var disposed = 0;
      Widget preview() => YouTubePreview(
        video: _video,
        playerBuilder: (_, _, _) => _FakePlayer(onDisposed: () => disposed++),
      );
  ...
  testWidgets('native preview always offers safe external playback', (
    tester,
  ) async {
  ...
  testWidgets(
    'YouTube embeds retain title/provider/poster without a native decoder',
    (tester) async {
      await tester.pumpWidget(
        _host(
          AccordEmbedBox(
  ...
  testWidgets('untrusted provider HTML is never treated as native video', (
    tester,
  ) async {
    await tester.pumpWidget(
      _host(
