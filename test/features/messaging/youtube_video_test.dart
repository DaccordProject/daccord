...
void main() {
  ...
  group('YouTube URL normalization', () {
    ...
    test('normalizes timestamps and strips all untrusted player options', () {
      final video = YouTubeVideo.parse('https://youtu.be/$id?t=1h2m3s&origin=https://evil.test&autoplay=1&list=playlist')!;
      expect(video.startSeconds, 3723);
      final player = Uri.parse(video.playerUrl('https://chat.example:8443/path'));
      ...
      expect(player.queryParameters, {
        ...
        'origin': 'https://chat.example:8443', 'start': '3723',
      });
      expect(video.canonicalUrl, 'https://www.youtube.com/watch?v=$id&t=3723s');
    });

    for (final entry in {'?t=90': 90, '?start=20&t=30': 20, '#t=1m5s': 65, '?t=-1': 0, '?t=NaN': 0, '?t=9000000000': 2147483647, '?t=999999999999999999h': 2147483647}.entries) {
      test('timestamp ${entry.key}', () {
        expect(YouTubeVideo.parse('https://youtu.be/$id${entry.key}')!.startSeconds, entry.value);
      });
    }

    for (final source in [
      null, '', 'javascript:alert(1)', 'file:///tmp/video',
    ...
    test('player rejects opaque/local origins', () {
      ...
      for (final origin in ['null', 'file:///tmp/page', 'javascript:alert(1)', 'https://user@chat.test']) {
  ...
  test('HTML provider URLs never reach a native decoder', () {
    for (final url in ['https://youtube.com/watch?v=$id', 'https://youtu.be/$id', 'https://youtube.com/embed/$id', 'https://video.test/player', 'file:///video.mp4', 'https://user@video.test/video.mp4']) {
      expect(isDirectEmbedVideo(url), isFalse, reason: url);
    }
    expect(isDirectEmbedVideo('https://cdn.test/clip.MP4?signature=abc'), isTrue);
