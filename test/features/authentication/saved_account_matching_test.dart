...
void main() {
  ...
  test('endpoint identity normalizes spelling without crossing origins', () {
    for (final equivalent in [
      ...
      expect(
        AccordServer.sameEndpoint(equivalent, 'https://chat.example'),
        isTrue,
      );
    }
    expect(
      AccordServer.sameEndpoint('http://localhost:80/', 'http://LOCALHOST'),
      isTrue,
    );
    expect(
      AccordServer.sameEndpoint('http://localhost', 'https://localhost'),
      isFalse,
    );
    for (final different in [
      ...
      expect(
        AccordServer.sameEndpoint(different, 'https://chat.example'),
        isFalse,
      );
    }
    expect(
      AccordServer.sameEndpoint(
        'https://chat.example/Team',
        'https://chat.example/team',
      ),
      isFalse,
    );
  ...
  test(
    'saved account matches before any live connection has been restored',
    () async {
      ...
      expect(
        await auth.accountForBaseUrl('https://CHAT.example:443'),
        same(saved),
      );
  ...
  test(
    'a pending vault read does not report an absent saved account',
    () async {
      final saved = _session('https://chat.example', 'saved-user');
  ...
  test(
    'on-demand reconnect creates one client without changing active auth',
    () async {
      ...
      try {
        ...
        expect(
          container.read(connectionsControllerProvider).connections,
          hasLength(1),
        );
        ...
        store.active = active;
        expect(
          await auth.accountForBaseUrl(saved.server.baseUrl),
          same(active),
        );
