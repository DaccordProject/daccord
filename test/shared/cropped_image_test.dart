...
void main() {
  ...
  test('bounded icon output preserves transparent pixels and is real PNG', () async {
    final source = img.Image(width: 1254, height: 1254, numChannels: 4);
    img.fillRect(source, x1: 627, y1: 0, x2: 1253, y2: 1253,
        color: img.ColorRgba8(255, 0, 0, 255));
    final bytes = await prepareCroppedImage(Uint8List.fromList(img.encodePng(source)), maxDimension: 512);
  ...
  test('banner preparation keeps top and bottom of the selected crop', () async {
    final source = img.Image(width: 1600, height: 900);
    img.fillRect(source, x1: 0, y1: 0, x2: 1599, y2: 449,
        color: img.ColorRgb8(255, 0, 0));
    img.fillRect(source, x1: 0, y1: 450, x2: 1599, y2: 899,
        color: img.ColorRgb8(0, 0, 255));
    final bytes = await prepareCroppedImage(Uint8List.fromList(img.encodeJpg(source)), maxDimension: 1024);
    final output = img.decodePng(bytes)!;
  ...
  test('small images are not enlarged', () async {
    ...
    final bytes = await prepareCroppedImage(Uint8List.fromList(img.encodePng(source)), maxDimension: 1024);
    final output = img.decodePng(bytes)!;
