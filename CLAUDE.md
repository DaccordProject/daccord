...
- `Config` (`scripts/autoload/config.gd`) -- Persists server connection configs to `user://config.cfg`. Stores base URL, token, and guild name per server. Supports `add_server()`, `remove_server()`, `get_servers()`, `has_servers()`.
