...
# Preload (instead of class_name lookup) so the autoload doesn't race the
# global class registry at startup.
const Extractor := preload("res://scripts/autoload/updater_extractor.gd")
...
func _extract_update(archive_path: String) -> Dictionary:
	return Extractor.extract_update(archive_path)
