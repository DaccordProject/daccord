...
func build() -> VBoxContainer:
	var vbox: VBoxContainer = _page_vbox.call(tr("Profiles"))
	...
	var new_btn := SettingsBase.create_action_button(tr("New Profile"))
	new_btn.pressed.connect(_on_new_profile)
	btn_row.add_child(new_btn)

	var import_btn := SettingsBase.create_secondary_button(tr("Import Profile"))
...
func _refresh_profiles_list() -> void:
	...
	for p in prof_list:
		...
		popup.add_item(tr("Set Password"), 1)
		popup.add_item(tr("Export"), 2)
		if slug != "default":
			popup.add_separator()
			popup.add_item(tr("Delete"), 3)
		popup.add_separator()
		popup.add_item(tr("Move Up"), 4)
		popup.add_item(tr("Move Down"), 5)
...
func _show_rename_dialog(slug: String, current_name: String) -> void:
	...
	dlg.title = tr("Rename Profile")
	dlg.ok_button_text = tr("Rename")
...
func _on_import_profile() -> void:
	...
	fd.add_filter("*.daccord-profile; *.cfg", tr("Profile Files"))
	fd.file_selected.connect(func(path: String) -> void:
		fd.queue_free()
