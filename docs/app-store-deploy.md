...
- **A real-device cloud** (BrowserStack App Live, Sauce Labs, AWS Device Farm).
  ...
  A cloud session recording will not look like a file made by Control Centre.
  The provider records the streamed device surface in the desktop browser, so
  its MP4 may be low-resolution, carry a mouse pointer, omit the iOS status bar
  and home indicator, have no audio, and identify a generic FFmpeg/Chrome
  encoder. None of those properties says whether the device behind the stream
  was physical, and the MP4 itself carries no useful device provenance. Before
  filming, open the provider's **Device information** panel and retain a
  screenshot showing the iPhone model and iOS version. Name that model, OS,
  provider, and the installed Daccord build in the review reply. If the session
  did not run on a provider-labelled real device, post-processing cannot turn
  its recording into the physical-device evidence Apple requested.
...
Treat that weaker confirmation as a failed rehearsal, not as the final App
Review take: it proves local hiding, but not that the DM was flagged to anyone
who can moderate it. The submitted recording must finish this beat on **"Report
submitted"**. Test `POST /api/v1/reports` against the exact recording instance
before resetting the app and starting the final take.
...
For a real-device-cloud capture, make the provenance and the evidence map
explicit instead of asking the reviewer to infer either from the video:
...
The tracked files under `fastlane/metadata/ios/en-US` provide the stable marketing,
support and privacy URLs uploaded by the iOS submission lane. Release notes in
the same directory remain generated by `dist/app-store-release-notes.sh`.
Check those links before submission; their in-app equivalents live in
