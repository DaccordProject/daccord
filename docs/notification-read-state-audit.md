...
Audited 2026-09-07. Scope: channel and DM unread highlighting, mention counts,
...
Read-only inspection of the sibling `accordserver` repository found that
...
142 focused Flutter tests passed across channels, notifications, gateway read
events, message-pane loading/reading, voice chat parity and DM search. New tests
cover serialized acknowledgements, failures/reconnect retries, numeric cursor
ordering, duplicate events, delayed remote reads, app inactivity/resume, history
loading, server-specific mute settings, and notification dismissal races.
Targeted Flutter analysis reported no issues. Riverpod generation completed;
the existing generator warned that its analyzer supports Dart 3.10 while the
installed SDK uses 3.12.
