...
class InitializerNativeCallable {
  ...
  ///
  /// Returns the [WakeUpNativeCallable] associated with [ctx] so the caller can
  /// close it **after** `mpv_terminate_destroy` has joined the mpv core thread.
  /// Closing it any earlier aborts the process with "Callback invoked after it
  /// has been deleted" — see the note in [NativePlayer.dispose].
  WakeUpNativeCallable? dispose(Pointer<generated.mpv_handle> ctx) {
    _locks.remove(ctx.address);
    _eventCallbacks.remove(ctx.address);
    // Stop libmpv from scheduling any *new* wakeups, but do not close the
    // callable yet: an in-flight wakeup on the mpv core thread may still land.
