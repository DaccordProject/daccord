...
class LocalParticipant extends Participant<LocalTrackPublication> {
  ...
  Future<void> removePublishedTrack(String trackSid, {bool notify = true}) async {
    ...
    if (track != null) {
      ...
      // bonfire #68: skip native sender removal + renegotiation when leaking is
      // enabled. These run on the publisher PC during the (server-forced)
      // teardown that heap-corrupts on Linux; we leak the PC instead.
      if (sender != null && !kLiveKitSkipNativeRelease) {
        try {
