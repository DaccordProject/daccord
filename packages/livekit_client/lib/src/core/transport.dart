...
class Transport extends Disposable {
  ...
  Transport._(this.pc, this.connectOptions) {
    ...
    onDispose(() async {
      ...
      // bonfire #68: skip ALL native PeerConnection teardown (removeTrack,
      // close, dispose) when leaking is enabled. Closing/disposing the native PC
      // is the heap-overflow crash site on Linux during a (server-forced)
      // disconnect; we leak the PC instead (reclaimed at process exit).
      if (!kLiveKitSkipNativeRelease) {
        // Remove all senders
